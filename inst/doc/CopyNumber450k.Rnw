...
<<writecsv,eval=FALSE>>=
segment_list <- getSegments(mcds.f)
...
\Rcode{plotSample} takes the segmentized \Rcode{CNV450kSet} object and the index
of the sample to plot as input. For example, we
can see that sample 2 has no clear chromosomal
aberrations (Figure~\ref{fig:plotSample}).
\begin{figure} 
\begin{center}
<<plotSample,fig=TRUE,results=hide>>=
plotSample(mcds.f, 2, main="Genomic view of Sample 2")
@
\end{center} 
...
plotSample(mcds.f, 4, chr="chr9", main="Sample 4, chromosome 18 q deletion")
@
\end{center} 
