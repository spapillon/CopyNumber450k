...
<<options,echo=FALSE,results=hide,eval=FALSE>>=
options(width=70)
@
...
contain at least one sample whose associated Sample_Group phenotypic data value
is \Rcode{control}. A sample whose Sample_Group entry is not \Rcode{control} is
considered to be non-control.

In order to facillitate the use of this package, we prove 52 control samples in the \Rpackage{CopyNumber450kData}.  We strongly suggest that you use those
...
\Rcode{control} as Sample_Group).

<<createobject,eval=FALSE>>=
...
chip, can be clearly observed using the density plot. Additionnaly, coloring can
be based on different sample features. Refer to the method documentation for all
possible groupings.

\begin{figure} 
\begin{center}
<<densityplot,fig=TRUE,eval=FALSE>>=
par(mfrow=c(1,2))
plotDensity(mcds)
plotDensity(mcds, color.by="slide")
@
\end{center} 
\caption{Global sample intensity by array position and by slide}
...
that the samples follow the same distribution and is therefore useful in cases where one can expect gross chromosomal aberrations (e.g. cancer samples).
...
array position bias is removed.

\begin{figure} 
\begin{center}
<<funnormDensity,fig=TRUE,eval=FALSE>>=
plotPCA(mcds.q)
@
\end{center} 
...
normalization, the PCA plot clearly seperates two groups, namely male and female patients.

\begin{figure} 
\begin{center}
<<quannormPCA,fig=TRUE,eval=FALSE>>=
plotPCA(mcds.q)
@
\end{center} 
...
Let's use the quantile-normalized object to do so, as we do not expect global
aberrations in our samples.

<<segmentation,eval=FALSE>>=
...
<<writecsv,eval=FALSE,eval=FALSE>>=
...
sapply(1:sample_count, function(sample_index) {
            ...
            write.csv(segment_list[[sample_index]], file=paste(sample_name, ".csv", sep="."))
    })
@
...
\begin{figure} 
\begin{center}
<<plotSample,fig=TRUE,eval=FALSE>>= 
plotSample(mcds.q, 3)
@
\end{center} 
...
\begin{figure} 
\begin{center}
<<plotSampleFocal,fig=TRUE,eval=FALSE>>= 
plotSample(mcds.q, 3, chr="chr6")
@
\end{center} 
