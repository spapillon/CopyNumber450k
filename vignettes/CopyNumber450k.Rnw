...
input. In this example, we can see that the assayed sample has a
fairly large deletion on chromosome 1 (Figure~\ref{fig:plotSample}).
...
plotSample(mcds.q, 1, chr="chr1",
        main="Sample 1, focal deletion on chromosome 1", ylim=c(-.25,.25))
@
\end{center} 
