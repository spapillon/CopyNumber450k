...
setMethod("plotSample", signature("CNVObject"), function(object, index, chr, start, 
    end) {
    ...
    myPlot <- plot(range(start, end), range(yMin, yMax), type = "n", xaxt = "n", 
...
setMethod("plotDensity", signature("CNVObject"), function(object, color.by, color.function, 
    ...
    myPlot <- plot(density(int_matrix), col = coloring$sample.colors[1], ylim = c(0, 
