######################################################## Data extraction for FunNorm450k Jean-Philippe Fortin Sept 24 2013 Code in
######################################################## development
...
extractFromRGSet450k <- function(RGSet) {
    ...
    for (i in 1:5) {
        mQuantiles[[i]] <- apply(m[indList[[i]], ], 2, function(x) quantile(x, probs = probs, 
            na.rm = T))
        betaQuantiles[[i]] <- apply(beta[indList[[i]], ], 2, function(x) quantile(x, 
            probs = probs, na.rm = T))
        methQuantiles[[i]] <- apply(meth[indList[[i]], ], 2, function(x) quantile(x, 
            probs = probs, na.rm = T))
        unmethQuantiles[[i]] <- apply(unmeth[indList[[i]], ], 2, function(x) quantile(x, 
            probs = probs, na.rm = T))
        cnQuantiles[[i]] <- apply(cn[indList[[i]], ], 2, function(x) quantile(x, 
            probs = probs, na.rm = T))
    }
    ...
    return(list(mQuantiles = mQuantiles, betaQuantiles = betaQuantiles, methQuantiles = methQuantiles, 
