...
setMethod("filterSNPProbes",
          signature("CNVObject"),
          function(object) {
            
  annotation <- probesAnnotation(object)
  usedProbes(object) <- usedProbes(object) & annotation$Probe_SNPs == "" & 
...
setMethod("filterVariantProbes",
          signature("CNVObject"),
          function(object,
                   variance_centile) {
                 
  # The variance_centile argument is the % of the most variant sites you want
  # to remove.
...
setMethod("normalize",
  ...
  if (method == "functional") {
    object@intensity_matrix  <-
        functionalNormalization(cnMatrix      = intensityMatrix(object)[usedProbes(object), ], 
                                extractedData = RGSetSummary(object),
                                predictedSex  = sampleSexes(object))
  }
  else if (method == "quantile") {
    object@intensity_matrix  <-
        quantileNormalization(cnMatrix     = intensityMatrix(object)[usedProbes(object), ], 
                              predictedSex = sampleSexes(object))
  }
  
  # Since the returned matrix has dropped the FALSE usedProbes,
  # remove other objects accordingly.
...
setMethod("buildSegments",
  ...
  # TODO: This should be in the imports/require section of NAMESPACE
  require(DNAcopy)
  # ---
  
  CNA.object <- CNA(cases_log2,
                    ordered(annotation$CHR),
                    as.numeric(annotation$MAPINFO),
                    data.type="logratio",
  ...
  object@segments <- formatSegments(segment.smoothed.CNA.object,
                                    case_intensity,
                                    control_intensity,
                                    probesAnnotation(object),
                                    verbose = verbose)
  object
})
...
setMethod("createFilters",
  ...
  filters <- lapply(segments(object),
                    function(sample) {
                      as.numeric(sample[,'num.mark']) >= tick.threshold &
