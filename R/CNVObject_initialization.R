...
setMethod("show", signature("CNVObject"), function(object) str(object))

setMethod("filterSNPProbes", signature("CNVObject"), function(object) {
    annotation <- probesAnnotation(object)
    usedProbes(object) <- usedProbes(object) & annotation$Probe_SNPs == "" & annotation$Probe_SNPs_10 == 
        ""
    object
})
...
setMethod("createFilters", signature("CNVObject"), function(object, tick.threshold, 
    pvalue.threshold, breakpoints) {
